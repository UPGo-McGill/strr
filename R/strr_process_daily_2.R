...
#'
#' \code{strr_process_daily_2} takes raw daily tables from AirDNA and cleans them
...
strr_process_daily_2 <- function(daily, property, keep_cols = FALSE,
                                 quiet = FALSE) {
  ...
  count <- created <- dif <- full_count <- high <- low <-  price <-
    property_ID <- scraped <- status <- PID_split <- NULL
  ...
  ## Split by first three digits of property_ID
  ...
  daily_list <-
    split(daily, by = "PID_split", keep.by = FALSE) %>%
    helper_table_split()

  helper_progress_message(
    "(3/6) Table split for processing.", .type = "close")


  ### Compress processed data file #############################################

  helper_progress_message(
    "(4/6) Beginning processing, using {helper_plan()}.",
    .type = "progress")

  output <-
    daily_list %>%
    future_map(strr_process_daily_helper,
...
#' Helper function to process daily file
#'
#' \code{strr_process_daily_helper} takes a processed host table and generates a
#' compressed version.
#'
#' TKTK
#'
...
strr_process_daily_helper <- function(daily) {
