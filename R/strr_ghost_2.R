...
strr_ghost_2 <- function(
  ...
  if (multi_date) {
    ...
    points[, date_grid := lapply(date_grid, function(x) {
      setDT(x)[Var1 <= Var2]
    })]
  ...
  points <-
    ...
    furrr::future_map(~{
      .x %>%
        ghost_cluster_2(distance, min_listings) %>%
        ghost_intersect_2(distance, min_listings) %>%
        ghost_intersect_leftovers_2(distance, min_listings)
    },
...
ghost_cluster_2 <- function(points, distance, min_listings) {
  ...
  points[, predicates := lapply(predicates, function(pred) {
    map(seq_along(pred), ~{
  ...
  points[, predicates := lapply(predicates,
  ...
  points[, data := mapply(pred_fun, data, predicates, SIMPLIFY = FALSE)]

  # Add condition for nrow 1 so nested data frame doesn't get unlisted
  if (nrow(points) == 1) {points[, predicates := NULL]
  } else {
    points <- points[, .(data = unlist(data, recursive = FALSE)),
                     by = "host_ID"]
  }
...
ghost_combine_2 <- function(buffers, predicates, n) {
...
ghost_stepwise_intersect_2 <- function(buffers, min_listings) {
  ...
  combinations <- ghost_combine_2(buffers, predicates, n)
...
ghost_intersect_2 <- function(points, distance, min_listings) {
  ...
  points[, intersects := map(buffers, ghost_stepwise_intersect_2,
                             min_listings)]
...
ghost_identify_leftovers_2 <- function(points, min_listings) {
...
ghost_intersect_leftovers_2 <- function(points, distance, min_listings) {
  ...
  leftovers <- ghost_identify_leftovers_2(points, min_listings)
  ...
  while (nrow(leftovers) > 0) {
    ...
    leftover_outcome <- ghost_intersect_2(leftovers, distance, min_listings)
